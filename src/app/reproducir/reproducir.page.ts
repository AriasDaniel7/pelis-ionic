...
@Component({
  ...
  templateUrl: './reproducir.page.html',
  styleUrls: ['./reproducir.page.scss'],
})
export class ReproducirPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.peliculas.forEach(peli => {
      if (peli.titulo == nombre) {
        this.URLVideo = this.sanitizer.bypassSecurityTrustResourceUrl(peli.url);
        this.templatePeli = peli;
      }
    });
